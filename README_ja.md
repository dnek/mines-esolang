...
2つの異なる盤面マスの行番号同士及び列番号同士の差の絶対値がそれぞれ `1` 以下であれば、それらの盤面マスは**隣接**している。
...
**操作リスト**は1個以上の**操作**を要素として持つ単方向の循環リストである。
...
盤面の行番号 `i` の**行**はすべての行番号 `i` の**盤面マス**を列番号順に繋げて表される。
...
**クリック操作**はその座標を `(i, j)` として、`i` を表す**未正規化列番号**と**マウスボタン**と `j` を表す**未正規化行番号**を順に繋げて表される。
