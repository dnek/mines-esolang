...
Minesはプログラムがマインスイーパーのゲームプレイのように振る舞う言語である。
...
これらはそれぞれ盤面の幅及び高さで割った非負剰余に変換される。

- `0,0`: "左上のマスを左クリック"
- `-1,-1`: "右下のマスを左クリック"
- `12;-34`: "左から12番目、下から34番目のマスを右クリック"

プログラム中に現れる `\t\r\f\v` の各文字及び `#` から行末までの文字列は無視される。
つまりプログラムはコメントを含むことができる。
